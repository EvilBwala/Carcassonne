...
class PointsCollector:
    ...
    def count_city_points(game_state: CarcassonneGameState, city: City):
        ...
        for position in city.city_positions:
            ...
            tile: Tile = game_state.get_tile(coordinate.row, coordinate.column)
            if tile.cathedral:
                has_cathedral = True
            coordinates.add(coordinate)

        tiles: [Tile] = [game_state.get_tile(x.row, x.column) for x in coordinates]
