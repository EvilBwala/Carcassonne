...
class TilePositionFinder:
    ...
    def possible_playing_positions(game_state: CarcassonneGameState, tile_to_play: Tile) -> [PlayingPosition]:
        ...
        placed_positions = list(game_state.board.keys())

        # Candidate slots: empty cells adjacent to any placed tile — O(placed_tiles × 4).
        candidate_slots = set()
        for r, c in placed_positions:
            for dr, dc in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                nr, nc = r + dr, c + dc
                if game_state.get_tile(nr, nc) is None:
                    candidate_slots.add((nr, nc))

        playing_positions = []
        for (row_index, column_index) in candidate_slots:
            ...
            for tile_turns in range(4):
                if TileFitter.fits(
                    tile_to_play.turn(tile_turns),
                    top=top, bottom=bottom, left=left, right=right,
                    game_state=game_state,
                ):
                    playing_positions.append(
                        PlayingPosition(
                            coordinate=Coordinate(row=row_index, column=column_index),
                            turns=tile_turns,
                        )
                    )
