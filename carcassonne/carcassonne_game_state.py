...
class CarcassonneGameState:
    ...
    def __init__(
            ...
            starting_position: Coordinate = Coordinate(0, 0),
            empty=False,
    ):
        if not empty:
    ...
    def to_json(self):
        board_json = {
            f"{r},{c}": tile_registry.tile_to_json(tile)
    ...
    def from_json(data: Dict[str, Any]):
        ...
        if isinstance(board_data, dict):
            state.board = {
                tuple(int(x) for x in key.split(",")): tile_registry.tile_from_json(val)
            ...
            for r, row in enumerate(board_data):
                for c, tile in enumerate(row):
                    if tile is not None:
                        state.board[(r, c)] = tile_registry.tile_from_json(tile)
